...
public class SetOfNaturalsTest {
    ...
    @Test
    public void testIntersectForFulIntersection() {
        SetOfNaturals setE = new SetOfNaturals();
        for (int i = 5; i < 50; i++) {
            setE.add(i * 10);
        }
        assertTrue(setC.intersects(setE), "full intersection should work for C and E");
        assertTrue(setE.intersects(setC), "full intersection should work for C and E");

    }

    @Test
    public void testIntersectForPartialIntersection() {
        assertTrue(setB.intersects(setD), "partial intersection should work for B and D");
        assertTrue(setD.intersects(setB), "partial intersection should work for B and D");

    }

    @Test
    public void testSize() {
        assertEquals(0, (new SetOfNaturals().size()));
        setD = new SetOfNaturals();
        for (int i = 1; i < 10; i++) {
            setD.add(i * 10);
        }
        assertEquals(9, setD.size());
    }
